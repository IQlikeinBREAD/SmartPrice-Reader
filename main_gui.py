...
class OCRApp(ctk.CTk):
    def __init__(self):
        ...
        # ===== LAYOUT (kamera z lewej, panel z prawej) =====
    ...
    def create_layout(self):
        ...
        ctk.CTkLabel(
            self.frame_controls,
            text="Wykryta Cena (PLN):"
        ).pack(anchor="w", padx=20)

        self.result_box = ctk.CTkEntry(
            self.frame_controls,
            font=("Arial", 30, "bold"),
            justify="center"
        )
        ...
        self.currency_box.configure(state="disabled")  # nie edytowalne przez użytkownika
    ...
    def process_current_frame(self):
        if self.detector is None or self.reader is None:
            self.status_label.configure(
                text="Modele jeszcze się ładują!",
                text_color="red"
            )
            return
        ...
        # ===== Wyświetlanie ceny w PLN i walutach =====
        EXCHANGE_RATES = {
            "EUR": 4.75,
            "USD": 4.40,
            "CHF": 4.80,
